...
void RbTree<T>::RbDelete(RbNode<T>* z) {
    ...
    if (yOriColor == BLACK) RbDeleteFixup(x);
    cnt--;
    delete z;
    z = nullptr;
}
        ...
        for (int i = 0; i < myvec.size(); i++) {
            if (myvec[i] != T->Nil) {
                ...
                if (myvec[i]->left != T->Nil) {
                    if (myvec[i]->color == RED && myvec[i]->left->color == RED)
                        std::cerr << "error with rbtree" << std::endl;
                    vectemp.push_back(myvec[i]->left);
                }
                if (myvec[i]->right != T->Nil) {
                    if (myvec[i]->color == RED && myvec[i]->right->color == RED)
                        std::cerr << "error with rbtree" << std::endl;
...
int main() {
    {
        ...
        std::random_shuffle(
                va.begin(),
                va.end());    //,std::mt19937{std::random_device{}()}

        std::clock_t a_start = std::clock();
        for (int     i     = 0; i < va.size(); i++) { T->RbInsert(va[i]); }
        std::clock_t a_end = std::clock();
        std::cout << (a_end - a_start) * 1000 / CLOCKS_PER_SEC << "ms"
                  << std::endl;
        std::ofstream ofs(
                "/home/caoqi/GITs/study diary/c++learning/RBtree/res2.txt");
