...
class RbNode {
  public:
    RbNode() {}
    RbNode(T _val)
            : val(_val), color(RED), left(NULL), right(NULL), pre(NULL) {}
    RbNode(bool _color, T _val, RbNode* _pre)
            : color(_color), val(_val), left(NULL), right(NULL), pre(NULL) {}
...
class MySet {
  public:
    ...
    void clear() { rb->RbClear(); }
    int count(const T& t) {
        if (this->findhlp(t).first)
            return 1;
        else
            return 0;
    }
    void print() { std::cout << rb; }

  private:
    std::pair<bool, RbNode<T>*> findhlp(const T& t) { return rb->RbFind(t); }
    RbTree<T>* rb;
};