...
struct RoutineInfo {
    ...
    RoutineInfo(size_t size) {
        ...
        stacksize                        = 0;
        if (stackbase != NULL) stacksize = size;
...
int CreateCoroutine(RoutineHandler handler, void *param) {
    ...
    memcpy(info->stackbase, stackBackup,
           info->stacksize);    // restore the stack
...
void *CoroutineStart(void *pRoutineInfo) {
    ...
    if (!setjmp(info.buf)) {
        ...
        if (stackBackup) memcpy(stackBackup, info.stackbase, info.stacksize);
...
void Switch() {
    ...
    if (!setjmp(current->buf)) { longjmp((*routines.begin())->buf, 1); }
